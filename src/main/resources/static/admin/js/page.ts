...
import queryString = Common.queryString;
import mapToObject = Common.mapToObject;
...
function renderPage(placeholder, dataCount, pageSize ) {
    ...
    let pageIndex = queryString().get('page') as string;
    ...
    page.addListener('pageChanged', function() {
        ...
        window.location.href = '/admin/video_list?'+ $.param(query);
    });
    page.render();
}
